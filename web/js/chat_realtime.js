...
var chat_realtime = (j, k, l, m, n, imageDir, domain) => {
    ...
    function headingHTML(avatar, name, status,nameidmember) {
        document.getElementsByClassName('you')[0].src = avatar;
        document.getElementById('heading-name-meta').innerHTML = (nameidmember=='')?name:nameidmember;
    ...
    function messageHTML(a, bottom) {
        ...
        if (a.name == m) {
            ...
            if (uKe == "Public") {
                ...
                //b += `<a class="message-username" style="color:#${col} !important">${a.name}${a.nameidmember}</a>`;
                b += `<a class="message-username" style="color:#${col} !important">${a.nameidmember}</a>`;
            }
            b += `      <div class="message-text">${image != '' ? `<a title="Zoom" href="${imageDir}/${image}" class="placeholder"><img class="imageDir" src="${imageDir}/${image}"/></a>` : ''}${urltag(htmlEntities(a.message))}</div>`;
            b += `      <span class="message-time pull-right">${timeToWords(a.date)}</span>`;
            b += '  </div>';
    ...
    function sideOneHTML(a) {
        let b = "";
        var named=(getname(a.name));
        b += `<div class="row sideBar-body" data-tipe="users" data-name="${named}" data-login="${a.date}" data-avatar="${a.avatar}" data-status="${a.status}" id="${a.name}">`;
        ...
        b += `                <span class="name-meta">${named}</span>`;
        b += '      </div>';
    ...
    function sideTwoHTML({login, avatar, status, name,nameidmember}) {
        let b = "";
        b += `<div class="row sideBar-body" data-name="${nameidmember}" data-tipe="users" data-login="${login}" data-avatar="${avatar}" data-status="${status == 'online' ? 'online' : 'offline'}" id="${name}">`;
        b += '<div class="col-sm-3 col-xs-3 sideBar-avatar">';
        b += '  <div class="avatar-icon">';
        ...
        b += '  <div class="row">';
        b += '  <div class="col-sm-8 col-xs-8 sideBar-name">';
        b += `      <span class="name-meta">${nameidmember}</span>`;
        b += '  </div>';
        b += '  <div class="col-sm-4 col-xs-4 pull-right sideBar-time">';
        ...
        if (files) {
            for (let i = 0, f; f = files[i]; i++) {
                if (/(jpe?g|png|gif)$/i.test(f.type)) {
                    ...
                    r.readAsDataURL(f);
                }else if(/(wav|mp4|mp3)$/i.test(f.type)){
                  const v = new FileReader();
                    v.onload = (({type, name, size}) => ({target}) => {
                        const base64Img = target.result;
                        const binaryImg = convertDataURIToBinary(base64Img);
                        const x = tampungImg.length;
                        const videos=window.URL.createObjectURL(f);
                        const fileName = makeid(name.split('.').pop());
                        tampungImg[x] = {
                            name: fileName,
                            type: type,
                        ...
                        //console.log(tampungImg[x]);
                        $('#reviewImg').append(`<div data-idx="${fileName}"  style="display: block;" class='video-prev' class="pull-right"><video style="width: 80%;" src="${videos}" class="video-preview" controls="controls"/></div>`);
                    })(f);
                    v.readAsDataURL(f);
                  ...
                  //           binary: Array.from(binaryImg)
                  //       };
                  // $('#reviewImg').append(`<div style="display: block;" class='video-prev' class="pull-right"><video style="width: 80%;" src="${videos}" class="video-preview" controls="controls"/></div>`);
                }else{
    ...
    $('body').on('click', '.side-one .sideBar-body', function() {
        ...
        const nameidmember = $(this).data('name');
        ...
        uKe = a;
        uTipe = tipe;
        headingHTML(av, a, st,nameidmember);
        chatMysql(tipe, a, a => {
            messages = a;
            no = 0;
            document.getElementsByClassName('messages')[0].innerHTML = "";
    ...
    $('body').on('click', '.side-two .sideBar-body', function() {//cliked sidebar2
        messages = [];
        const a = $(this).attr('id');
        const tipe = $(this).data('tipe');
        const nameidmember = $(this).data('name');
    ...
    $('body').on('click', '.message-username', function() {//public
        messages = [];
        const a = $(this).html();
        const tipe = $(`body .side-two #${a}`).data('tipe');
        const nameidmember = $(`body .side-two #${a}`).data('name');
    ...
    document.getElementById("scroll").addEventListener("click", () => {//scroll bar right
        const cc = $('#conversation');
        const dd = cc[0].scrollHeight;
        cc.animate({
            scrollTop: dd
    ...
    $('body').on('click', '.heading-home', function() {//to public rooms
        uKe = $(this).attr("id");
        uTipe = $(this).data("tipe");
        nameidmember = $(this).data("name");
        headingHTML($(this).data("avatar"), uKe, uTipe,name);
        inbox = 0;
        $(".inbox-status").hide();
        $('.side-one .sideBar-body').removeClass("active");
        chatMysql('rooms', 'Public', a => {
            ...
            $('.placeholder').magnificPopup({//images popup
                type: 'image',
                closeOnContentClick: true,
                mainClass: 'mfp-img-mobile',
                image: {
